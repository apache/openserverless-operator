...
def spool_template(template, out, data):
    if template.startswith("__"):
        template = template[2:]

    rendered = expand_template(template, data)

    with open(out, "w") as f:
        f.write(rendered)

    logging.info("📄 spool_template: generato %s da template %s", out, template)
    logging.debug("📄 Contenuto generato:\n%s", rendered)
