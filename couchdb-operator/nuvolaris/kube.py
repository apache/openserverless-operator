...
def kubectl(*args, namespace="nuvolaris", input=None, jsonpath=None, debugresult=True):
    ...
    logging.info(f"Error: kubectl {cmd} input='{input}' output='{output}' error='{error}'")
...
# apply an object
def apply(obj, namespace=None):
    try:
        return kubectl("apply", "-f", "-", namespace=namespace, input=obj)
    except Exception as e:
        kind = obj.get("kind", "").lower()
        name = obj["metadata"]["name"]
        if kind == "job" and "field is immutable" in str(e):
            print(f"⚠️  Job {name} esiste già con template diverso, lo elimino.")
            try:
                kubectl("delete", kind, name, "-n", namespace)
            except Exception as delete_error:
                print(f"⚠️  Errore nel delete del Job {name}: {delete_error}")
            return kubectl("apply", "-f", "-", namespace=namespace, input=obj)
        raise e
