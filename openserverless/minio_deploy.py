...
import openserverless.minio_ingress as minio_ingress
...
def create_nuv_storage(data):
    """
    Creates openserverless MINIO custom resources
    """
    logging.info(f"*** configuring MINIO storage for openserverless")
    ...
    if(res):
        ...
        logging.info("*** adding openserverless MINIO data bucket")
        res = minioClient.make_bucket("openserverless-data")                
        bucket_policy_names.append("openserverless-data/*")

        if(res):
            openwhisk.annotate("s3_bucket_data=openserverless-data")

        logging.info("*** adding openserverless MINIO static public bucket")
        res = minioClient.make_public_bucket("openserverless-web")             
        bucket_policy_names.append("openserverless-web/*")
        ...
        if(res):
            ...
            if(content_path):
                logging.info(f"uploading example content to openserverless-web from {content_path}")
                res = minioClient.upload_folder_content(content_path,"openserverless-web")
            else:
        ...
        logging.info("*** configured MINIO storage for openserverless")
