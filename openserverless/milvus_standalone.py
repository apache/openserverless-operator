...
from openserverless.seaweedfs_util import SeaweedfsClient
...
def patchEntries(data: dict):
    ...
    kust += kus.patchGenericEntry("Secret", "openserverless-milvus-etcd-secret", "/data/username",
                                  util.b64_encode(data['milvus_etcd_username']))
    kust += kus.patchGenericEntry("Secret", "openserverless-milvus-etcd-secret", "/data/password",
                                  util.b64_encode(data['milvus_etcd_password']))
    
    kust += kus.patchGenericEntry("Secret", "openserverless-milvus-s3-secret", "/stringData/accesskey",
                                  data['milvus_s3_username'])
    kust += kus.patchGenericEntry("Secret", "openserverless-milvus-s3-secret", "/stringData/secretkey",
                                  data['milvus_s3_password'])

    
    kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus", "/spec/storageClassName",
                                  data['storageClass'])
    kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus", "/spec/resources/requests/storage",
                                  f"{data['size']}Gi")

    if not data["slim"]:
        kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus-zookeeper", "/spec/storageClassName",
                                    data['storageClass'])
        kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus-zookeeper",
                                    "/spec/resources/requests/storage", f"{data['zookeeper_size']}Gi")

        kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus-bookie-journal", "/spec/storageClassName",
                                    data['storageClass'])
        kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus-bookie-journal",
                                    "/spec/resources/requests/storage", f"{data['bookie_journal_size']}Gi")

        kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus-bookie-ledgers", "/spec/storageClassName",
                                    data['storageClass'])
        kust += kus.patchGenericEntry("PersistentVolumeClaim", "openserverless-milvus-bookie-ledgers",
...
def create(owner=None):
    ...
    if res:
        ...
        util.wait_for_pod_ready(
            r"{.items[?(@.metadata.labels.app\.kubernetes\.io\/instance == 'openserverless-milvus')].metadata.name}")

        milvus_api_host = cfg.get("milvus.host", "MILVUS_API_HOST", "openserverless-milvus")
...
def create_default_milvus_database(data):
    """
    Creates openserverless MILVUS custom resources
    """
    logging.info("*** configuring MILVUS database for openserverless")
    adminClient = MilvusAdminClient()
    res = adminClient.setup_user("openserverless", data["openserverless_password"], "openserverless")

    if (res):
        _annotate_nuv_milvus_metadata(data)
        logging.info("*** configured MILVUS database for openserverless")
        return True
...
def _annotate_nuv_milvus_metadata(data):
    ...
    try:
        milvus_service = util.get_service(
            r"{.items[?(@.metadata.labels.app\.kubernetes\.io\/instance == 'openserverless-milvus')]}")
        if (milvus_service):
            milvus_host = f"{milvus_service['metadata']['name']}.{milvus_service['metadata']['namespace']}.svc.cluster.local"
            password = data["openserverless_password"]

            openwhisk.annotate(f"milvus_host={milvus_host}")
            openwhisk.annotate(f"milvus_token=openserverless:{password}")
            openwhisk.annotate("milvus_db_name=openserverless")
        ...
        logging.error(f"failed to annotate MILVUS for openserverless: {e}")
        return None
