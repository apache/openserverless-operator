...
class OpenServerlessMetadata:
    _data = {}

    def __init__(self):
        password = cfg.get('openserverless.password') or "nuvpassw0rd"
        salted_password = bu.hash_password(password)

        self._data = {
            "login":"openserverless",
    ...
    def _store_safely_from_cm(self,metadata_key,json_path):
        try: 
            value = util.get_value_from_config_map("openserverless", json_path)
            if value:
    ...
    def _configure_from_cm(self):
        ...
        """        
        self.add_metadata("AUTH", cfg.get('openwhisk.namespaces.openserverless'))
