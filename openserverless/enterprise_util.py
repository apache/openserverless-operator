...
def extract_host(srv_address_url):
    ...
    'zookeeper-0.zookeeper'
    >>> extract_host("zookeeper-0.zookeeper.openserverless.svc.cluster.local:2181")
    'zookeeper-0.zookeeper.openserverless.svc.cluster.local'
    """
...
def getEnterpriseControllerConfigData():
    data = {
        ...
        "kafka_host": cfg.get("openserverless.kafka.url") or  "kafka:9092",
        "zookeeper_url": cfg.get('openserverless.zookeeper.url') or "zookeeper-0.zookeeper:2181",
...
def getEnterpriseInvokerConfigData():
    data = {
        ...
        "kubernetes_action_namespace":cfg.get('configs.invoker.kubernetes.action_namespace') or "openserverless",
        "kubernetes_user_pod_affinity_enabled":cfg.get('configs.invoker.kubernetes.user_pod_affinity_enabled') and "true" or "false",
        "kubernetes_user_pod_affinity_key":cfg.get('configs.invoker.kubernetes.user_pod_affinity_key') or "openserverless-role",
...
def get_prometheus_config_data():
    data = {
        "name":"openserverless-prometheus-server",
        "pod_anti_affinity_name":"openserverless-prometheus-server", 
        "pvcName":"openserverless-prometheus-server-pvc",
...
def get_am_config_data():
    data = {
        "name":"openserverless-prometheus-alertmanager",
        "pod_anti_affinity_name":"openserverless-prometheus-alertmanager",      
        "pvcName":"storage-openserverless-prometheus-alertmanager",
        "pvcAccessMode":"ReadWriteOnce",
        "size": cfg.get("monitoring.alert-manager.volume-size") or 2,
        "storageClass": cfg.get("openserverless.storageclass"),
        "slack": cfg.get("monitoring.alert-manager.slack.enabled") or False,
        "slack_channel_name": cfg.get("monitoring.alert-manager.slack.slack_channel_name") or '#monitoring-openserverless',
...
def get_kafka_config_data():
    data = {
        ...
        "storage_class": cfg.get("openserverless.storageclass") or "standard"
    }
    util.common_affinity_tolerations_data(data)
    return data    