...
loader = FileSystemLoader(["./openserverless/templates", "./openserverless/files"])
...
def init():
    # load openserverless config from the named crd
    config = os.environ.get("OPENSERVERLESS_CONFIG")
    if config:
        import logging
    ...
    db = openserverless.couchdb_util.CouchDB()
...
def init_users_metadata(db):
    """
    Add a new Openwhisk Couchdb database to host openserverless user relevant informations
    """
    dbn = "users_metadata"
    res = check(db.wait_db_ready(60), "wait_db_ready", True)
...
def init_compactions_config(db):
    """
    Activate the compactions config for the openserverless related databases
    """
    res = check(db.wait_db_ready(60), "wait_db_ready", True)
