...
#
#
# Deploys mongodb for openserverless using operator or standalone
# implementation.
#
...
import openserverless.mongodb_operator as operator
import openserverless.mongodb_standalone as standalone
...
def get_mdb_pod_name():
    ...
    pod_name_jsonpath = useOperator and "{.items[?(@.metadata.labels.app == 'openserverless-mongodb-svc')].metadata.name}" or "{.items[?(@.metadata.labels.app == 'openserverless-mongodb')].metadata.name}"
...
def update_system_cm_for_mdb():
    logging.info("*** annotating configuration for mongodb openserverless user")
    try:        
        mdb_service = util.get_service("{.items[?(@.metadata.name == 'openserverless-mongodb-svc')]}")
        if(mdb_service):
            mdb_pod_name = get_mdb_pod_name()                
            ...
            username = urllib.parse.quote(data['mongo_openserverless_user'])
            password = urllib.parse.quote(data['mongo_openserverless_password'])
            auth = f"{username}:{password}"
            
            mdb_url = f"mongodb://{auth}@{mdb_pod_name}.{mdb_service_name}.{mdb_ns}.svc.cluster.local:27017/openserverless?connectTimeoutMS=60000"
            openwhisk.annotate(f"mongodb_url={mdb_url}")
            logging.info("*** saved annotation for mongodb openserverless user")            
    except Exception as e:
...
def create_db_user(ucfg: UserConfig, user_metadata: UserMetadata):
    ...
    try:
        ...
        pod_name = util.get_pod_name("{.items[?(@.metadata.labels.app == 'openserverless-mongodb')].metadata.name}")
