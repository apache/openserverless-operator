...
def enrich_ferretdb_data(data):
     ...
     data['dir']= "/state"            
     data['storageClass']=cfg.get("openserverless.storageclass")
     data['name']="openserverless-mongodb"
     data['container']="ferretdb"
     data['applypodsecurity']=util.get_enable_pod_security()
     
     # use openserverless postgres db as default configuration for FERRETDB
...
def create(owner=None):
    ...
    if res:
        util.wait_for_pod_ready("{.items[?(@.metadata.labels.app == 'openserverless-mongodb')].metadata.name}")
...
def get_ferret_pod_name():
    return util.get_pod_name("{.items[?(@.metadata.labels.app == 'openserverless-mongodb')].metadata.name}")    

def update_system_cm_for_mdb(data):
    logging.info("*** annotating configuration for ferretdb openserverless user")
    try:        
        mdb_service = util.get_service("{.items[?(@.metadata.name == 'openserverless-mongodb-svc')]}")
        if(mdb_service):
            ...
            username = urllib.parse.quote(data['postgres_openserverless_user'])
            password = urllib.parse.quote(data['postgres_openserverless_password'])
            auth = f"{username}:{password}"
            
            mdb_url = f"mongodb://{auth}@{mdb_pod_name}.{mdb_service_name}.{mdb_ns}.svc.cluster.local:27017/openserverless?connectTimeoutMS=60000&authMechanism=PLAIN"
            openwhisk.annotate(f"mongodb_url={mdb_url}")
            logging.info("*** saved annotation for ferretdb openserverless user")            
    except Exception as e:
        logging.error(f"failed to build mongodb_url for openserverless database: {e}")  
...
def _add_mdb_user_metadata(user_metadata, data):
    ...
    something like "mongodb://{namespace}:{auth}@openserverless-mongodb-0.openserverless-mongodb-svc.openserverless.svc.cluster.local:27017/{database}?connectTimeoutMS=60000"}
    """ 
    try:
        mdb_service = util.get_service("{.items[?(@.metadata.name == 'openserverless-mongodb-svc')]}")        
...
def create_db_user(ucfg: UserConfig, user_metadata: UserMetadata):
    ...
    try:
        ...
        pod_name = util.get_pod_name("{.items[?(@.metadata.labels.app == 'openserverless-postgres')].metadata.name}")      
