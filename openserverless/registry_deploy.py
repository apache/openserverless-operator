...
from openserverless.secret_htpasswd_data import SecretHtpasswordData
from openserverless.secret_imagepull_data import ImagePullSecretData
...
def create_external_registry(data, owner=None):
    ...
    path_to_template_yaml = registrySecret.render_template("openserverless")
    ...
    path_to_template_yaml = registryPullSecret.render_template("openserverless")
...
def _annotate_registry_metadata(data):
    """
    annotate openserverless configmap with entries for registry connectivity REGISTRY_ENDPOINT, REGISTRY_USERNAME, RESIGTRY_PASSWORD    
    """ 
    try:
        ...
        logging.error(f"failed to annotate registry_host for openserverless: {e}")
        return None     
...
def create_registry_ingress(data, owner=None):
    ...
    """    
    if cfg.get('openserverless.kube') == 'openshift':           
        return deploy_registry_route(data)
    else:
        return deploy_registry_ingress(data)
    
def deploy_registry_route(data, namespace="openserverless"):
    """
    ...
    route.with_service_name("openserverless-registry-svc")
    route.with_service_kind("Service")
    route.with_service_port("5000")
    route.with_context_path("/")

    logging.info("*** configuring registry route for service openserverless-registry-svc:5000")
...
def deploy_registry_ingress(data, namespace="openserverless"):
    ...
    ingress.with_service_name("openserverless-registry-svc")
...
def delete_registry_ingress(owner=None, namespace="openserverless"):
    """
