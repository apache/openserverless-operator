...
class MockKube:
    """
    >>> from openserverless.testutil import *
    >>> m = MockKube()
    >>> m.invoke()
    >>> m.config("", "ok")
    >>> m.invoke()
...
def set_apihost_from_kubeconfig(cfg):
    ...
    # the openserverless apihost is the same as the kube api server
    ...
    kube = cfg.get("openserverless.kube")
    if kube is None:
        return
    if kube in ["k3s", "microk8s"]:
        cfg.put("openserverless.apihost", hostname)
    if kube == "openshift":
        hostname1 =  ".".join(["openserverless", "apps"] + hostname.split(".")[1:])
        cfg.put("openserverless.apihost", hostname1)
...
def generate_ow_uid():
    """
        >>> import openserverless.testutil as util        
