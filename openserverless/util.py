...
import openserverless.apihost_util as apihost_util
import openserverless.config as cfg
import openserverless.kube as kube
import openserverless.template as template
...
def get_ingress_namespace(runtime):
    ...
    >>> import openserverless.config as cfg
    >>> cfg.put('openserverless.ingresslb','auto')
    True
    >>> get_ingress_namespace('microk8s')
    'ingress'
    >>> get_ingress_namespace('kind')
    'ingress-nginx'
    >>> cfg.put('openserverless.ingresslb','ingress-nginx-azure/ingress-nginx-controller')
    True
    >>> get_ingress_namespace('kind')
    'ingress-nginx-azure'
    """
    ingresslb_value = cfg.get('openserverless.ingresslb') or 'auto'
...
def get_ingress_service_name(runtime):
    ...
    'service/ingress-nginx-controller'
    >>> cfg.put('openserverless.ingresslb','ingress-nginx-azure/ingress-nginx-controller-custom')
    True
    >>> get_ingress_service_name('kind')
...
def get_ingress_class(runtime):
    ...
    """
    ingress_class = cfg.get('openserverless.ingressclass') or 'auto'
...
# wait for a pod name
@nuv_retry()
def get_pod_name(jsonpath,namespace="openserverless"):
...
def wait_for_pod_ready(pod_name_jsonpath, timeout="600s", namespace="openserverless"):
    try:
...
def get_redis_config_data():
    ...
    prefix = cfg.get("redis.openserverless.prefix") or "openserverless:"
    ...
    data = {
        ...
        "password":cfg.get("redis.openserverless.password") or "s0meP@ass3",
        "prefix": prefix,
...
def get_service(jsonpath,namespace="openserverless"):
...
def get_postgres_config_data():
    data = {
        ...
        'postgres_openserverless_user': "openserverless",
        'postgres_openserverless_password': cfg.get('postgres.openserverless.password') or "s0meP@ass3",
...
def get_postgres_backup_data():
    data = {
        ...
        'storageClass': cfg.get('openserverless.storageclass'),
        'schedule': cfg.get('postgres.backup.schedule') or '30 * * * *',
        'name': 'openserverless-postgres-backup',
        'dir':'/var/lib/backup',
        'container':'openserverless-postgres-backup'
        }
    postgres_affinity_tolerations_data(data)
    return data
...
@nuv_retry()
def wait_for_service(jsonpath,namespace="openserverless"):
...
def get_apihost_from_config_map(namespace="openserverless"):
...
def get_value_from_config_map(namespace="openserverless", path='{.metadata.annotations.apihost}'):
...
def get_runtimes_json_from_config_map(namespace="openserverless", path=r'{.data.runtimes\.json}'):
...
def get_storage_static_config_data():
    data = {
        "name":"openserverless-static",
        "container":"openserverless-static",
        "size":1,
        "storageClass": cfg.get('openserverless.storageclass'),
        "dir":"/var/cache/nginx",
        "applypodsecurity": get_enable_pod_security()
    ...
    if cfg.get('components.seaweedfs'):
        ...
        data['storage_url']=f"http://{seaweedfs_api_host}.openserverless.svc.cluster.local:{seaweedfs_api_port}"
...
def common_affinity_tolerations_data(data):
    data["affinity"] = cfg.get('openserverless.affinity') or False
    data["tolerations"] = cfg.get('openserverless.tolerations') or False
...
def postgres_affinity_tolerations_data(data):
    common_affinity_tolerations_data(data)
    data["pod_anti_affinity_name"] = "openserverless-postgres"
...
def postgres_backup_affinity_tolerations_data(data):
    common_affinity_tolerations_data(data)
    data["pod_anti_affinity_name"] = "openserverless-postgres-backup"
...
@nuv_retry()
def get_pod_name_by_selector(selector, jsonpath, namespace="openserverless"):
    """
    get pods matching the given selector filtering them using the given jsonpath.
    param: selector (eg app="openserverless-postgres")
...
@nuv_retry()
def get_service_by_selector(selector,jsonpath,namespace="openserverless"):
    """
...
def get_object_storage_class():
    ...
    """
    storage_class = kube.kubectl("get", "storageclass", jsonpath="{.items[?(@.parameters.objectStoreName=='openserverless-s3-store')].metadata.name}")
    if(storage_class):
        return storage_class[0]
...
def get_object_storage_rgw_url():
    ...
    """
    rgw_urls = kube.kubectl("get", "cephobjectstores",namespace="rook-ceph",jsonpath="{.items[?(@.metadata.name=='openserverless-s3-store')].status.info.endpoint}")
    if(rgw_urls):
        return rgw_urls[0]
...
def get_object_storage_rgw_srv_name():
    ...
    Get the object store RGW service URL, to be used to configure the static nginx services when running on top of a CEPH OBJECT STORE
    """
    rgw_urls = kube.kubectl("get", "svc",namespace="rook-ceph",jsonpath="{.items[?(@.metadata.labels.rgw=='openserverless-s3-store')].metadata.name}")
    if(rgw_urls):
        return rgw_urls[0]
...
def get_object_storage_rgw_srv_http_port():
    ...
    Get the object store RGW service URL, to be used to configure the static nginx services when running on top of a CEPH OBJECT STORE
    """
    rgw_ports = kube.kubectl("get", "svc",namespace="rook-ceph",jsonpath="{.items[?(@.metadata.labels.rgw=='openserverless-s3-store')].spec.ports[?(@.name=='http')].port}")
    if(rgw_ports):
        return rgw_ports[0]
...
def get_cosi_config_data():
    data = {
        ...
        "object_store_name": cfg.get('cosi.object_store_name') or "openserverless-s3-store",
...
def etcd_affinity_tolerations_data(data):
    common_affinity_tolerations_data(data)
    data["pod_anti_affinity_name"] = "openserverless-etcd"

def get_etcd_initial_clusters(name: str, replicas = 1):
    """ Calculate the proper setup for ETCD initial clusters
    >>> print(get_etcd_initial_clusters("openserverless-etcd"))
    openserverless-etcd-0=http://openserverless-etcd-0.openserverless-etcd-headless.openserverless.svc.cluster.local:2380
    >>> print(get_etcd_initial_clusters("openserverless-etcd",2))
    openserverless-etcd-0=http://openserverless-etcd-0.openserverless-etcd-headless.openserverless.svc.cluster.local:2380,openserverless-etcd-1=http://openserverless-etcd-1.openserverless-etcd-headless.openserverless.svc.cluster.local:2380
    """
    ...
    for idx in range(replicas):
        ...
        etc_initial_clusters += f"{name}-{idx}=http://{name}-{idx}.{name}-headless.openserverless.svc.cluster.local:2380"
...
def get_etcd_config_data():
    ...
    data = {
        "applypodsecurity":get_enable_pod_security(),
        "name": "openserverless-etcd",
        "container": "openserverless-etcd",
...
def milvus_standalone_affinity_tolerations_data(data):
    common_affinity_tolerations_data(data)
    data["pod_anti_affinity_name"] = "openserverless-milvus"
    data["name"] = "openserverless-milvus-standalone"
    data["container-name"] = "openserverless-milvus"
    return data
...
def get_milvus_config_data():
    data = {
        ...
        'milvus_bucket_prefix': 'milvus/openserverless-milvus',
        ...
        'storageClass': cfg.get('openserverless.storageclass'),
        'etcd_replicas':get_etcd_replica(),
        'etcd_container': 'openserverless-etcd',
        'milvus_root_password': cfg.get('milvus.password.root') or "An0therPa55",
        'openserverless_password': cfg.get('milvus.openserverless.password') or "Nuv0therPa55",
...
def get_registry_config_data():
    ...
    data['repoSvcHostname'] = "openserverless-registry-svc:5000"
...
def get_seaweedfs_config_data():
    data = {
        ...
        "seaweedfs_nuv_user": cfg.get('seaweedfs.openserverless.user') or "openserverless",
        "seaweedfs_nuv_password": cfg.get('seaweedfs.openserverless.password') or "zuf+tfteSlswRu7BJ86wekitnifILbZam1KYY3TG",
