...
def create(owner=None):
    ...
    mkust += kus.patchGenericEntry("Secret","postgres-openserverless-secret","/stringData/superUserPassword",data['postgres_root_password'])
    mkust += kus.patchGenericEntry("Secret","postgres-openserverless-secret","/stringData/replicationUserPassword",data['postgres_root_replica_password'])        
    mkust += kus.patchGenericEntry("Secret","postgres-openserverless-secret","/stringData/openserverlessUserPassword",data['postgres_openserverless_password'])     
    ...
    util.wait_for_pod_ready("{.items[?(@.metadata.labels.app == 'openserverless-postgres')].metadata.name}")

    if(res):
        util.wait_for_service("{.items[?(@.metadata.labels.replicationRole == 'primary')]}")        

    if data['backup']:
        logging.info("*** activating openserverless-postgres backup")
...
def update_system_cm_for_pdb(data):
    """"
    Annotate the default ConfigMAp with default OpenServerless user parameteres. The default user openserverless is setup by the Postgres setup script
    therefore it is not explicitly created by this operator.
    """
    logging.info("*** annotating configuration for postgres openserverless user")
    try:        
        pdb_service = util.get_service_by_selector("app=openserverless-postgres","{.items[?(@.metadata.labels.replicationRole == 'primary')]}")
        ...
        if(pdb_service):             
            ...
            username = "openserverless"
            database = "openserverless"
            password = urllib.parse.quote(data['postgres_openserverless_password'])
            ...
            logging.info("*** saved annotation for postgres openserverless user")            
    except Exception as e:
        logging.error(f"failed to build postgres data for openserverless database: {e}")
...
def _add_pdb_user_metadata(ucfg:UserConfig, user_metadata: UserMetadata):
    """
...
def create_db_user(ucfg: UserConfig, user_metadata: UserMetadata):
    ...
    try:
        ...
        pod_name = util.get_pod_name_by_selector("app=openserverless-postgres","{.items[?(@.metadata.labels.replicationRole == 'primary')].metadata.name}")
...
def patch(status, action, owner=None):
    ...
    try:
        ...
        if  action == 'create':
            msg = create(owner)
            operator_util.patch_operator_status(status,'postgres','on')            
        else:
            msg = delete(owner)            
            operator_util.patch_operator_status(status,'postgres','off')
