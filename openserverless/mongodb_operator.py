...
#
# Containers can get the openserverless mongodb connection string
# deployed from the openserverless-operator using something similar to
#containers:
# - name: test-app
#   env:
...
#      valueFrom:
#        secretKeyRef:
#          name: openserverless-mongodb-openserverless-openserverless
...
# kubectl -n openserverless port-forward service/openserverless-mongodb-svc 27017:27017
#
...
def create(owner=None):
    ...
    nuv_user = cfg.get('mongodb.openserverless.user') or "openserverless"
    nuv_pwd = cfg.get('mongodb.openserverless.password') or "s0meP@ass3"
    ...
    if( pod_name ):
        ...
        tpl_filter =  ["mongodb-auth.yaml","mongodb-auth-openserverless.yaml","mongodb.yaml"]
        if exposed: 
            logging.info("*** including mongodb service for localhost access")
            tpl_filter.append("mongodb-svc.yaml")
        

        mkust = kus.patchTemplates("mongodb-operator-deploy", ["mongodb-auth.yaml","mongodb-auth-openserverless.yaml","mongodb-config.yaml"], data)    
        ...
        util.wait_for_pod_ready("{.items[?(@.metadata.labels.app == 'openserverless-mongodb-svc')].metadata.name}")
    else:
