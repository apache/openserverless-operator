...
def endpoint(response: dict, item: dict):
    ...
    """
    if(item['path']=='spec.components.tls' or item['path']=='spec.openserverless.apihost'):
...
def evaluate_differences(response: dict, differences: list):
    ...
    openserverless operator need to deploy/undeploy
    """
...
def detect_component_changes(kopf_diff):
    ...
    Will return a list of items reporting the specific action to be done on any openserverless operator managed component
...
def evaluate_wsku_differences(response: dict, differences: list):
    ...
    openserverless operator wsku handler needs to patch.
    """
    for d in differences:
