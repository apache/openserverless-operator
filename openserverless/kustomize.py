...
import openserverless.kustomize as nku
...
def kustomize(where, *what, templates=[], data={}):
    """Test kustomize
    >>> import openserverless.kustomize as ku
...
# the openserverless operator needs to delete a component
def build(where):
    dir = f"deploy/{where}"
    return _build(dir)
...
def patchGenericEntry(kind, name, path, value, op="replace",apiVersion="v1"):
    """
    >>> print(patchGenericEntry("Secret","postgres-openserverless-secret", "/stringData/superUserPassword","xyz"), end='')
    - target:
        version: v1
        kind: Secret
        namespace: openserverless
        name: postgres-openserverless-secret
      patch: |-
        - op: replace
