...
from openserverless.ip_util import IpUtil
...
def assign_protocol(runtime_str, url):
    ...
    Setting inside the whisk.yaml the attribute openserverless.protocol will forse always to use the
    ...
    if cfg.exists("openserverless.protocol") and cfg.get("openserverless.protocol") in ["http","https"]:
        url = url._replace(scheme = cfg.get("openserverless.protocol"))    
    else:
...
def calculate_apihost(runtime_str,apiHost=None):
    ...
    if cfg.exists("openserverless.apihost") and not 'auto' == cfg.get('openserverless.apihost'):
        url =  url._replace(netloc = ensure_host(cfg.get("openserverless.apihost")))
    else:
        url =  url._replace(netloc = "miniops.me" )
    if cfg.exists("openserverless.apiport"):
        url = url._replace(netloc = f"{url.hostname}:{cfg.get('openserverless.apiport')}")
...
def get_apihost(runtime_str):
    ...
    if runtime_str in ["k3s","microk8s", "openshift"]:
        if(cfg.exists("openserverless.apihost") and 'auto' == cfg.get('openserverless.apihost') and not is_load_balanced_kube(runtime_str)):
            machine_ip = IpUtil().get_public_ip()
            logging.info(f"openserverless.apihost in auto mode. Will use machine public ip address {machine_ip}")
...
def extract_hostname(url):
    ...
    'localhost'
    >>> extract_hostname('https://openserverless.org')    
    'openserverless.org'
    """
...
def extract_port(url):
    ...
    8080
    >>> extract_port('https://openserverless.org')
    """
    parsed_url = urllib.parse.urlparse(url)
...
def split_hostname_port(url):
    ...
    ('localhost', 8080)
    >>> split_hostname_port('https://openserverless.org')
    ('openserverless.org', None)
    """
    parsed_url = urllib.parse.urlparse(url)
...
def append_prefix_to_url(url,prefix):
    ...
    >>> append_prefix_to_url("http://openserverless.dev:8080","www")    
    'http://www.openserverless.dev:8080'
    >>> append_prefix_to_url("http://www.openserverless.dev:8080","www")
    'http://www.openserverless.dev:8080'
    >>> append_prefix_to_url("http://openserverless.dev:8080",None)
    'http://openserverless.dev:8080'
    >>> append_prefix_to_url("https://openserverless.dev","www")
    'https://www.openserverless.dev'
    """
    try:
        if not prefix:
            return url
...
def add_suffix_to_url(url,suffix):
    ...
    >>> add_suffix_to_url("http://openserverless.dev:8080","svc.cluster.local")    
    'http://openserverless.dev.svc.cluster.local:8080'
    >>> add_suffix_to_url("http://openserverless.dev.svc.cluster.local:8080","svc.cluster.local")
    'http://openserverless.dev.svc.cluster.local:8080'
    >>> add_suffix_to_url("http://openserverless.dev:8080",None)
    'http://openserverless.dev:8080'
    >>> add_suffix_to_url("https://openserverless.dev","svc.cluster.local")
    'https://openserverless.dev.svc.cluster.local'
    """
    try:
        if not suffix:
            return url
