...
import openserverless.couchdb as cdb
import openserverless.couchdb_util as couchdb_util
...
def save_user_metadata(user_metadata: UserMetadata):
    ...
    logging.info(f"Storing OpenServerless metadata for {metadata['login']}")

    try:
        db = couchdb_util.CouchDB()
        return _add_metadata(db, metadata)
    except Exception as e:
        logging.error(f"failed to store OpenServerless metadata for {metadata['login']}. Cause: {e}")
        return None


def save_openserverless_metadata(openserverless_metadata: OpenServerlessMetadata):
    """
    Add openserverless user metadata into the internal CouchDB 
    """
    metadata = openserverless_metadata.get_metadata()
    logging.info(f"Storing OpenServerless metadata for {metadata['login']}")
...
def delete_user_metadata(login):
    logging.info(f"removing OpenServerless metadata for user {login}")
    ...
    try:
        ...
        logging.warn(f"OpenServerless metadata for user {login} not found!")
        return None
    except Exception as e:
        logging.error(f"failed to remove OpenServerless metadata for user {login}. Reason: {e}")
        return None


def update_user_metadata_password(login, password):
    logging.info(f"updating OpenServerless password for user {login}")
    ...
    try:
        ...
        logging.error(f"failed to update credentials for quota OpenServerless user {login}. Reason: {e}")
        return None


def update_user_metadata_quota(login, quota: list):
    logging.info(f"updating OpenServerless quota for user {login}")
    ...
    try:
        ...
        logging.error(f"failed to update quota for OpenServerless user {login}. Reason: {e}")
        return None
