...
from openserverless.ingress_data import IngressData
from openserverless.route_data import RouteData

def create(owner=None):
    logging.info("*** configuring openserverless nginx static provider")
    ...
    util.wait_for_pod_ready("{.items[?(@.metadata.labels.app == 'openserverless-static')].metadata.name}")
    create_nuv_static_ingress(runtime, owner)
    logging.info("*** configured openserverless nginx static provider")
    return res

def create_nuv_static_ingress(runtime, owner=None):
    """"
    Deploys the static ingresses for the openserverless user
    """
    ...
    if runtime == 'openshift':           
        res = deploy_content_route_template("openserverless",bucket_name, apihost_url)
        if should_create_www:
            res += deploy_content_route_template("www-openserverless",bucket_name,apihost_util.append_prefix_to_url(apihost_url,"www"))
        return res
    else:
        res = deploy_content_ingress_template("openserverless",bucket_name,apihost_url)
        if should_create_www:
            res += deploy_content_ingress_template("www-openserverless",bucket_name,apihost_util.append_prefix_to_url(apihost_url,"www"))
        return res

def static_ingress_name(namespace, default="apihost"):
    return namespace == "openserverless" and f"{default}-static-ingress" or f"{namespace}-static-ingress"
...
def deploy_content_route_template(namespace, bucket, url):
    ...
    content.with_service_name("openserverless-static-svc")
...
def delete_by_owner():
    ...
    logging.info(f"delete openserverless nginx static provider: {res}")
    return res
...
def delete_nuv_ingresses():
    logging.info("*** deleting openserverless static ingresses")
    runtime = cfg.get('openserverless.kube')
    ingress_class = util.get_ingress_class(runtime)
    apihost_url = apihost_util.get_apihost(runtime)
    hostname = apihost_util.extract_hostname(apihost_url)
    ...
    try:
        res = ""
        if(runtime=='openshift'):
            route_name = static_route_name("openserverless")
            res = kube.kubectl("delete", "route",route_name)

            if should_delete_www:
                route_name = static_route_name("www-openserverless")
                res += kube.kubectl("delete", "route",route_name)
            return res

        if(ingress_class == 'traefik'):            
            middleware_name = static_middleware_ingress_name("openserverless")
            res += kube.kubectl("delete", util.get_traefik_middleware_resource(),middleware_name)
            
            if should_delete_www:
                middleware_name = static_middleware_ingress_name("www-openserverless")
                res += kube.kubectl("delete", util.get_traefik_middleware_resource(),middleware_name)

        ingress_name = static_ingress_name("openserverless")
        res += kube.kubectl("delete", "ingress",ingress_name)

        if should_delete_www:
            ingress_name = static_ingress_name("www-openserverless")
            res += kube.kubectl("delete", "ingress",ingress_name) 
