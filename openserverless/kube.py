...
# default namespace is openserverless, you can change with keyword arg namespace
# default output is text
# if you specify jsonpath it will filter and parse the json output
# returns exceptions if errors
def kubectl(*args, namespace="openserverless", input=None, jsonpath=None, debugresult=True, timeout=None):
...
def configMap(name, **kwargs):
    """
    >>> import openserverless.kube as kube, openserverless.testutil as tu
...
# delete an object
def delete(obj, namespace="openserverless"):
    # tested with apply
...
# apply an object
def apply(obj, namespace="openserverless"):
    if not isinstance(obj, str):
        obj = json.dumps(obj)
    return kubectl("apply", "-f", "-", namespace=namespace, input=obj)

# apply an expanded template
def applyTemplate(name, data, namespace="openserverless"):
    obj = tpl.expand_template(name, data)
    return kubectl("apply", "-f", "-", namespace=namespace, input=obj)

# delete an expanded template
def deleteTemplate(name, data, namespace="openserverless"):
    obj = tpl.expand_template(name, data)
    return kubectl("delete", "-f", "-", namespace=namespace, input=obj)

def get(name, namespace="openserverless"):
    try:
        return json.loads(kubectl("get", name, "-ojson", namespace=namespace))
    except:
        return None

def get_pods(selector, namespace="openserverless"):
    """
...
def wait(name, condition, timeout="600s", namespace="openserverless"):
    try:
...
# patch an object
def patch(name, data, namespace="openserverless", tpe="merge"):
    if not type(data) == str:
        data = json.dumps(data)
    res = kubectl("patch", name, "--type", tpe, "-p", data)
    return res

def scale_sts(name, replicas, namespace="openserverless"):
    try:
...
def rollout(name, namespace="openserverless"):
    try:
...
def detect_kind():
    try:
        is_kind = kubectl("get","node/openserverless-control-plane",
                          namespace=None,
                          jsonpath='{.metadata.labels.openserverless\\.io/kube}')
