...
import openserverless.userdb_util as userdb
import openserverless.config as cfg
import openserverless.util as ut
import openserverless.whisk_actions_deployer as system
import openserverless.redis as redis

from openserverless.openserverless_metadata import OpenServerlessMetadata

def annotate_operator_components_version():
    """
    This functions scans for pod matching the annotations whisks.openserverless.org/annotate-version: "true"
    and uses pod metadata.labels.name and spec.containers.image to annotate the global config map cm/config
    """
    try:
        logging.info("**** annotating openserverless operator component versions")
        pods = kube.kubectl("get","pods",jsonpath=r"{.items[?(@.metadata.annotations.whisks\.openserverless\.org\/annotate-version)]}",debugresult=False)
        ...
        for pod in pods:
            if(pod['metadata'].get('labels') and pod['metadata']['labels'].get('name')):
                ...
                logging.warn("**** found a pod with whisks.openserverless.org/annotate-version without metadata.labels.name attribute")
                logging.warn(pod)
        
        logging.info("**** completed annotation of openserverless operator component versions")       
    except Exception as e:
        logging.error(e)

def update_openserverless_metadata():
    try:
        logging.info("**** persisting openserverless metadata")
        nuv_metadata = OpenServerlessMetadata()
        nuv_metadata.dump()
        userdb.save_openserverless_metadata(nuv_metadata)
        logging.info("**** openserverless metadata successfully persisted")
    except Exception as e:
        logging.error(e)        
...
def whisk_post_resume(name):
    ...
    - restore redis openserverless namespace if redis is active
    - annotate operator deployed components version
    ...
    if cfg.get("components.redis"):
        redis.restore_openserverless_db_user()
