...
def api_ingress_name(namespace, ingress="apihost"):
    return namespace == "openserverless" and ingress or f"{namespace}-{ingress}-api-ingress"

def api_route_name(namespace,route="apihost"):
    return namespace == "openserverless" and route or f"{namespace}-{route}-api-route"
...
def create(owner=None):
    ...
    if runtime == 'openshift':
        res = deploy_info_route(apihost,"openserverless")
        return deploy_api_routes(apihost,"openserverless",should_create_www)
    else:
        res = deploy_info_ingress(apihost,"openserverless")
        return deploy_api_ingresses(apihost,"openserverless",should_create_www)

def delete(owner=None):
    """
    undeploys ingresses for openserverless apihost
    """    
    logging.info(f"*** removing ingresses for openserverless apihost")
