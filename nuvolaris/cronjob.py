...
def patch(status, action, owner=None):
    ...
    try:
        ...
        if  action == 'create':
            msg = create(owner)
            operator_util.patch_operator_status(status,'cron','on') 
        else:
            msg = delete(owner)
            operator_util.patch_operator_status(status,'cron','off') 

        logging.info(msg)        
        logging.info(f"*** hanlded request to {action} cron") 
    except Exception as e:
        logging.error('*** failed to update cron: %s' % e)
        operator_util.patch_operator_status(status,'cron','error')      